...
import com.elytradev.opaline.block.BlockDistiller;
...
public class RenderDistiller extends TileEntitySpecialRenderer<TileEntityDistiller> {
    ...
    public void render(TileEntityDistiller te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        switch (ibs.getValue(BlockDistiller.FACING)) {
            case WEST:
        ...
        if (fluid != null) {
            ...
            renderCube(2.25f, 1, 6.25f, 3.5f, 2.5f*(fluid.amount/(float)te.tank.getCapacity()), 3.5f, tas, true, true, true);
        ...
        dummy.setItem(te.items.getStackInSlot(TileEntityDistiller.SLOT_LAPIS));
        rei.doRender(dummy, 1.4f, 0, 1, 0, partialTicks);
        dummy.setItem(te.items.getStackInSlot(TileEntityDistiller.SLOT_EXHAUSTED));
        rei.doRender(dummy, 1.4f, 0, 1, 0, partialTicks);
